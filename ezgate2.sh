...
#itk
url_itk="https://github.com/InsightSoftwareConsortium/ITK/releases/download/v5.2.1/InsightToolkit-5.2.1.tar.gz"
if validate_url url_itk; then
    ...
    tar -xvzf InsightToolkit-5.2.1.tar.gz
    rm InsightToolkit-5.2.1.tar.gz
else
...
#gate
url_gate=" https://github.com/OpenGATE/Gate/archive/refs/tags/v9.1.zip"
...
#cmake
url_gate=" https://github.com/Kitware/CMake/releases/download/v3.21.2/cmake-3.21.2-linux-x86_64.sh"
if validate_url url_gate; then
    echo "Url : $url_gate exists..."
    wget  https://github.com/Kitware/CMake/releases/download/v3.21.2/cmake-3.21.2-linux-x86_64.sh
    mv ~/cmake-3.21.2-linux-x86_64.sh /opt/
else
    echo "Url : $url_gate doesn't exists.."
    echo "The link seems to be broken, please contact the author to update the script"
    return
...
rm -r /opt/cmake-3.21.2-linux-x86_64.sh
echo "Installation of CMake-3.19.8 done."
echo -e "\n"
## Installation of ROOT
echo "Installation of root-6.24.00"
mkdir root_v6.24.06-build
cd $GPTH/GATE/root_v6.24.06-build
cmake ../root-6.24.06
cmake --build . -- -j$(nproc)
source $GPTH/GATE/root-6.24.06-build/bin/thisroot.sh
cd ..
...
echo -e "\n"
## Installation of ITK
echo "Installation of ITK 5.2.1 "
cd $GPTH/GATE/InsightToolkit-5.2.1
mkdir bin
cd bin
cmake -DITK_USE_REVIEW=ON -DBUILD_EXAMPLES=OFF -DBUILD_TESTING=OFF -DINSTALL_GTEST=ON -DITKV3_COMPATIBILITY=OFF -DITK_BUILD_DEFAULT_MODULES=ON -DITK_WRAP_PYTHON=OFF ..
make -j$(nproc)
sudo make install
cd ../..
...
cd Gate-9.1-build
cmake -DCMAKE_INSTALL_PREFIX=$GPTH/GATE/Gate-9.1-install -DBUILD_TESTING=OFF -DCMAKE_BUILD_TYPE=RELEASE -DGATE_DOWNLOAD_BENCHMARKS_DATA=OFF -DGATE_USE_DAVIS=OFF -DGATE_USE_ECAT7=OFF -DGATE_USE_GEANT4_UIVIS=ON -DGATE_USE_GPU=OFF -DGATE_USE_ITK=ON -DGATE_USE_LMF=OFF -DGATE_USE_OPTICAL=ON -DGATE_USE_RTK=OFF -DGATE_USE_STDC11=ON -DGATE_USE_SYSTEM_CLHEP=OFF -DGATE_USE_XRAYLIB=OFF -DGeant4_DIR=$GPTH/GATE/geant4.10.07.p01-install/lib/Geant4-10.07.1 -DITK_DIR=/usr/local/lib/cmake/ITK-5.2 -DROOTCINT_EXECUTABLE=$GPTH/GATE/root-6.24.01-build/bin/rootcint -DROOT_CONFIG_EXECUTABLE=$GPTH/GATE/root-6.24.01-build/bin/root-config $GPTH/GATE/Gate-9.1
make -j$(nproc)
make install
echo "Installation of Gate V9.1 done."
cd ..


## Export GATE environnment (path variable)
touch gate_env.sh
echo 'source' $GPTH'/GATE/root-6.24.01-build/bin/thisroot.sh' >> gate_env.sh
